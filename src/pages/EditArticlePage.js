...
const EditArticlePage = () => {
  ...
  const [articleId, setArticleId] = useState(null); // состояние для хранения идентификатора статьи
  ...
  useEffect(() => {
    const fetchArticleCategories = async () => {
      try {
        const response = await axios.get('http://192.168.1.6:8080/api/article_categories/get_all');
  ...
  useEffect(() => {
    if (articleData.genre) {
      const selectedGenre = genres.find(genre => genre.genreName === articleData.genre);
      if (selectedGenre) {
        fetchArticlesByGenre(selectedGenre.id);
      }
    }
  }, [articleData.genre, genres]);

  const fetchArticlesByGenre = async (genreId) => {
    try {
      const response = await axios.get(`http://192.168.1.6:8080/api/article/get_by_genre?genreId=${genreId}`);
  ...
  const fetchQuestionsByQuizId = async (quizId) => {
    try {
      console.log(`Fetching questions for quizId: ${quizId}`); // Отладочное сообщение
      const response = await axios.get(`http://192.168.1.6:8080/api/question/get_all?quizId=${quizId}`);
      const questions = response.data.questions
        .filter((question) => question.questionImages.length === 0)
        .map((question) => ({
          text: question.question,
          points: question.questionValue,
          answers: question.answers.map((answer) => answer.answer),
          correctAnswers: question.answers.map((answer) => answer.isCorrect),
          image: question.questionImages.length > 0 ? question.questionImages[0].questionImage : null,
        }));
      setQuestions(questions);
      console.log('Fetched questions:', questions); // Отладочное сообщение
    } catch (error) {
  ...
  const handleImageChange = (e) => {
    ...
    setIsModified(true); // Устанавливаем состояние изменений
  };
  ...
  const handleChangeArticleField = (field, value) => {
  ...
  const handleArticleSelect = async (articleId) => {
    ...
    if (selectedArticle) {
      setArticleData({
        category: categories.find(cat => cat.id === selectedArticle.artGenre)?.categoryName || '',
        type: types.find(type => type.id === selectedArticle.artGenre)?.typeName || '',
        genre: genres.find(genre => genre.id === selectedArticle.artGenre)?.genreName || '',
        title: selectedArticle.title,
        text: selectedArticle.text
      });
      setArticleId(selectedArticle.id); // Сохраняем идентификатор статьи
      console.log(`Selected article ID: ${selectedArticle.id}`); // Отладочное сообщение
      await fetchQuestionsByQuizId(selectedArticle.id);
    }
    setIsModified(false); // Сбрасываем состояние изменений
  };
  ...
  const handleUpdate = async () => {
    // Проверяем, что все поля статьи заполнены
    if (
      !articleData.title ||
      !articleData.text
    ) {
      alert("Пожалуйста, заполните все поля статьи.");
      return;
    }
    ...
    try {
      const response = await axios.post('http://192.168.1.6:8080/api/article/update', updateArticleRequest, {
        headers: {
          'Content-Type': 'application/json'
        }
      });

      // Обновляем название статьи в состоянии articles
      setArticles((prevArticles) =>
        prevArticles.map((article) =>
          article.id === articleId ? { ...article, title: articleData.title } : article
        )
      );

      console.log(response.data);
      setIsModified(false); // Сбрасываем состояние изменений после успешной отправки
    } catch (error) {
    ...
    <>
      ...
      <div className="add-article-container">
            ...
            <div className="form-block">
              ...
              <div className="form-group-container">
                <div className="form-group labels">
                  ...
                  <label>Выбрать статью:</label>
                </div>
                <div className="form-group selects">
                  <select value={articleData.category} onChange={(e) => handleChangeArticleField('category', e.target.value)}>
                  ...
                  <select value={articleData.type} onChange={(e) => handleChangeArticleField('type', e.target.value)}>
                  ...
                  <select value={articleData.genre} onChange={(e) => handleChangeArticleField('genre', e.target.value)}>
                  ...
                  <select onChange={(e) => handleArticleSelect(e.target.value)}>
            ...
            <div className="form-block">
              <div className="title-input">
                ...
                <input
                  ...
                  onChange={(e) => handleChangeArticleField('title', e.target.value)}
                />
              </div>
              <div className="title-input">
                ...
                <textarea
                  ...
                  onChange={(e) => handleChangeArticleField('text', e.target.value)}
                  style={{ resize: "none" }}
                />
              </div>
        ...
        <div className="navigation-buttons">
          ...
          >
            {isModified ? "Сохранить изменения" : "Сохранить статью и тест"}
          </button>
        </div>
      </div>
    </>
